...
在客户端管理页新增或编辑映射时，可以选择服务端映射端口的暴露方式：

- **对外监听**：沿用原有行为，映射端口监听服务端配置的 `HOST`。未选择端口暴露方式、旧版本创建的映射也默认使用此模式。
- **仅本机监听**：映射端口只监听 `127.0.0.1`，再由同一台主机上的 Nginx、Caddy 等反向代理转发到 `127.0.0.1:<映射端口>`，避免直接通过服务器 IP 和端口访问。
...
“仅本机监听”要求代理与服务端进程共享网络命名空间。它适用于 PM2/直接运行，或服务端使用 host 网络的 Docker 部署；bridge 容器中的 `127.0.0.1` 仅在容器内部可达。
...
选择“仅本机监听”时，映射端口只绑定宿主机 `127.0.0.1`，可直接供宿主机上的 Nginx/Caddy 使用，也不会在公网网卡上监听。
